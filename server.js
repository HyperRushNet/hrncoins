/**
 * server.js - geoptimaliseerd, geen cluster, hele nummers
 */
...
function pendingOpsSinceSnapshot() { return pendingOps.length; }
...
async function loadData() {
  ...
  if (snapshot?.users) snapshot.users.forEach(u => {
    const bal = u.infinite ? BigInt(0) : BigInt(u.balance || 0);
    users.set(u.name, { balance: bal, infinite: !!u.infinite });
  });
...
function getBalance(user) {
  ...
  return u.balance.toString(); // **hele getallen als string**
}
...
app.get("/deposit", (req,res) => {
  ...
  if (!user || !amt || isNaN(amt) || BigInt(amt)<=0) return res.status(400).send("Invalid");
  if (!users.has(user)) return res.status(404).send("User not found");
  applyOp({ type:"inc", user, amount: BigInt(amt) });
...
app.get("/transfer", (req,res) => {
  ...
  const amt = req.query.amount;
  if (!from || !to || !amt || isNaN(amt) || BigInt(amt)<=0) return res.status(400).send("Invalid");
  ...
  if (!s.infinite) applyOp({ type:"inc", user:from, amount: -BigInt(amt) });
  applyOp({ type:"inc", user:to, amount: BigInt(amt) });
  res.json({
...
(async()=>{ await loadData(); app.listen(PORT, ()=>console.log(`Server running on ${PORT}, Admin: ${ADMIN_CODE}`)); })();
